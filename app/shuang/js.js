...
/*
字库引用地址：https://github.com/BlueSky-07/Shuang/blob/master/src/dict.js
*/
...
mmd.init = function () {
  ...
  mmd.mainDict = [];
  let smObj, t;
  for (let sm in mmd.dict) {
    smObj = this.dict[sm];
    for (let ym in smObj) {
      t = smObj[ym];
      mmd.mainDict.push([sm + ym, Array.isArray(t) ? t[0] : t])
      //有点疑惑对象的遍历顺序问题，保险一点通过数组获取 index
      if (t ==='双') {
        mmd.shuangIndex = mmd.mainDict.length - 1;
      }
    }
  }
  //初始键位
  this.nowKey = this.keyboard.xiaohe;
}
...
mmd.keyboardOption = document.querySelector("#kb-option");

mmd.randChoice = function () {
  let len = this.mainDict.length - 1;
  if (len === 0) {
    mmd.init();
  }
  let rand = random(0, len)
  return this.mainDict.splice(rand, 1)[0];
}
...
mmd.searchDict = function (str, spKeyDict) {
  return spKeyDict[str];
}

mmd.zcs = ['zh', 'ch', 'sh'];

mmd.seekShuangpinKey = function (str) {
  let len = str.length, sm, ym;
  ym = this.searchDict(str, this.nowKey.nosm);
  if (ym) { return ym }
  sm = getStr(str, 0, 2);
  if (this.zcs.includes(sm)) {
    sm = this.searchDict(sm, this.nowKey.norm);
    ym = this.searchDict(getStr(str, 2, len), this.nowKey.norm);
    return ym ? (sm + ym) : false;
  } else {
    sm = getStr(str, 0, 1);
    ym = this.searchDict(getStr(str, 1, len), this.nowKey.norm);
...
let d_wz = document.querySelector("#d-wz");
let d_py = document.querySelector('#d-py')
...
mmd.init();
//初始状态下删除列表中双子
mmd.mainDict.splice(mmd.shuangIndex, 1);

inputbox.addEventListener('input', function () {
  if (this.value === shuang) {
    setTimeout(() => {
      this.value = '';
      ready();
    }, 45);
